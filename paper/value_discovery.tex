...
\paragraph{Subsequent experiments (v4 and v5).}
Two further experiments post-date the work reported above and inform the corrections
woven into this text. \emph{v4} pointed the instrument at an event-log deployment
simulator carrying an injected self-preserving utility: the value's structural
footprint there is the drive-without-intake of the \emph{actions the utility selects
for} (regime-graded, and visible in the auditor's log alone), not the outcome state the
...
\emph{v5} ran the signature side by side with a role-sufficiency formalism
independently derived by another PIBBSS mentor (roles as minimal internal
representations sufficient for named external targets; interventional at full
...
distributed value structures; the twin channel as a probe of value persistence
through records; and a synthesis instrument combining the signature's world-directed,
all-source intake with the strict stability gate and an explicit uniqueness pressure
(the v5 reversals define its requirements).
