...
Locating the Value Function of an Agent from Passive Observation\\
by its Empowerment--Plasticity Asymmetry}

\author[1]{Simon Beard}
...
which part plays the role of its \emph{value function}? We propose that this role has a
directional information-theoretic signature: values are the components that most
strongly drive the rest of the system while being least driven by it --- maximally
empowered and minimally plastic. This inverts a familiar assumption due to Omohundro
...
system does, we look for the value function in the \emph{informational structure} of
...
what lies inside them, and specifically which part of a discovered agent plays the
role of its values.
...
discovery: we are identifying the part that plays the value-function role. In the
simple systems studied here that part is a single planted goal bit; the same structural
signature could in principle locate richer value functions in richer systems.
...
The empowerment--plasticity signature locates a planted value function from passive
...
states; a value function distributed over several components is beyond it as it stands.
...
distributed value functions; and the twin channel as a probe of value persistence
through records.
...
decisions log. This draft is not being submitted for publication.
